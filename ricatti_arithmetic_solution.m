...
disp('Ricatti ODEs are:');
disp([eqP1;eqP2;eqP]);
% **********************************************************************
%   Equations eqP1, eqP2 and eqP are programmed into ricatti_odefun
...
conP2 = p2(100) == 100;
conP = p(100) == 0;
disp('Ricatti boundary conditions are:');
disp([conP1,conP2,conP]);
% **********************************************************************
%   Boundary conditions conP1, conP2 and conP are programmed into 
%   ricatti_bcfun function. Implemented at end of script.
% **********************************************************************

tmesh = 0:0.1:T;

% Guess BVP solutions. Guesses are constant functions. (???) 
ricatti_solinit = bvpinit(tmesh,[1;1;1]);
opts = bvpset('Stats', 'on'); % enable stats for bvp solvers.

% Solve system of ODEs with 4c solver.
disp('Solving Ricatti ODEs...'); disp(' ');
disp('4c solver says:');
ricatti_sol4c = bvp4c(@ricatti_odefun, @ricatti_bcfun, ...
    ricatti_solinit, opts);
ricatti_sol = ricatti_sol4c;

% Plot Costate Variables over time.
plot_solver_states(3,ricatti_sol,'A) Costate Variables over Time', ...
...
M = length(ricatti_sol.x)*repelemFactor;
P = zeros([2, 2, M]);

P(1,1,:) = repelem(ricatti_sol.y(1,:),repelemFactor);
P(2,2,:) = repelem(ricatti_sol.y(2,:),repelemFactor);
P(1,2,:) = repelem(ricatti_sol.y(3,:),repelemFactor);
P(2,1,:) = repelem(ricatti_sol.y(3,:),repelemFactor);
...
disp([eqP1;eqP2;eqP]);
% **********************************************************************
%   Equations eqP1, eqP2 and eqP are programmed into ricatti_odefun2
%   function. Function is implemented at end of script.
% **********************************************************************
...
disp('Solving Ricatti ODEs...'); disp(' ');
disp('4c solver says:');
ricatti_sol4c = bvp4c(@ricatti_odefun2, @ricatti_bcfun, ...
    ricatti_solinit, opts);
ricatti_sol = ricatti_sol4c;

% Plot Ricatti Variables over time.
plot_solver_states(3,ricatti_sol,'B) Costate Variables over Time', ...
        ['p11(t)';'p22(t)';'p12(t)'],line_width);
...
% ***********************************************************************
%
% A) BVP for Ricatti Solution to time varying system.
%       P1 is P11, P2 is P22, P3 is P12=P21.
function dPdt = ricatti_odefun(t,p)
    dPdt = zeros(3,1);
...
function bc = ricatti_bcfun(p0, ptf)
...
%       P1 is P11, P2 is P22, P3 is P12=P21.
function dPdt = ricatti_odefun2(t,p)
    dPdt = zeros(3,1);
    dPdt(1) = p(3)^2 - 1;
